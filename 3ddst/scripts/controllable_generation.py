...
def parse_args():
    ...
    parser.add_argument('--model_name', type=str, default='Flux.1-dev-Controlnet-Surface-Normals/control_normalbae')
    parser.add_argument('--data_path', type=str, default='/home/kaist/semyeong/data/static/3ddst')
    parser.add_argument('--data_name', type=str, default='image_diffusion')
    parser.add_argument('--synsets', type=str, nargs='+', default=['n04540053', 'n03777568'])
    ...
    for split in os.listdir(args.data_path):
        ...
        for synset in all_synsets:
            ...
            for model_id in all_model_ids:
                ...
                # canny_path = os.path.join(args.data_path, split, synset, model_id, 'cannyedge_render')
                # normal_path = os.path.join(args.data_path, split, synset, model_id, 'normal_render')
                ...
                for sample_idx in range(args.num_samples):
                    ...
                    while img_prompt_itr.hasnext():
                        ...
                        #     image=image_list, image_name=img_name_list, prompt=prompt_list, negative_prompt=negative_prompt_list, canny_path=canny_path)
                        results = model.forward(
                            image=image_list, image_name=img_name_list, prompt=prompt_list, negative_prompt=negative_prompt_list, ddim_steps=args.ddim_steps, scale=args.scale, canny_path=canny_path)
