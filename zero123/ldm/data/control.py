...
class ObjaverseDataText(Dataset):
    def __init__(self,
        root_dir,
        txt_path,
        caption_path=None,
...
class ObjaverseData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        target_RT = np.load(os.path.join(filename, 'annotation', '%03d.npy' % index_target), allow_pickle=True).item()['modelview_matrix']
        cond_RT = np.load(os.path.join(filename, 'annotation', '%03d.npy' % index_cond), allow_pickle=True).item()['modelview_matrix']
