...
class ControlLDM(LatentDiffusion):
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        if "text" not in cond:
            _latent_hint = cond["latent"]
            _x = torch.cat([x_noisy, _latent_hint], dim=1)
            
            eps = diffusion_model(x=_x, timesteps=t, context=cond["pose"], control=None, only_mid_control=self.only_mid_control)

        else:
            _latent_hint = cond["latent"]
            _x = torch.cat([x_noisy, _latent_hint], dim=1)
            control = self.control_model(x=x_noisy, hint=None, timesteps=t, context=cond["text"])
            
            for ctr_idx, ctr_scale in enumerate(self.control_scales):
                control[ctr_idx] = control[ctr_idx] * ctr_scale
            
            eps = diffusion_model(x=_x, timesteps=t, context=cond["pose"], control=control, only_mid_control=self.only_mid_control)
        return eps
    ...
    def get_unconditional_conditioning(self, batch_size, null_label=[""], image_size=512):
        cond = {}
        cond["pose"] = self.get_learned_conditioning(null_label, modality="txt").repeat(batch_size, 1, 1)
        cond["latent"] = torch.zeros([batch_size, 4, image_size // 8, image_size // 8]).to(self.device)
    ...
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, plot_denoise_rows=False, plot_diffusion_rows=False, unconditional_guidance_scale=9.0, unconditional_guidance_label=None, **kwargs):
        ...
        if unconditional_guidance_scale > 1.0:
            ...
            uc_pose = torch.zeros_like(c["pose"])
            uc_latent = torch.zeros_like(c["latent"]) # c_control
            uc_full = {"pose": uc_pose, "latent": uc_latent}
